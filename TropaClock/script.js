...
(function(){
    ...
    function tick(){
      ...
      if (now < startDate) {
        ...
        countdownTimeEl.textContent = `Starts in ${days}d ${pad(hours)}:${pad(minutes)}:${pad(seconds)}`;
        ...
        const minutes = Math.floor((diff / 60000) % 60);
        const seconds = Math.floor((diff / 1000) % 60);
        countdownTimeEl.textContent = `${days}d ${pad(hours)}:${pad(minutes)}:${pad(seconds)}`;
        countdownDateEl.textContent = `Until ${endDate.toLocaleString()}`;
      } else {
